...
const Dashboard = () => {
  ...
  // 🔄 Sync dashboard logs from localStorage
  const syncLogs = () => {
    try {
  ...
  // 🔁 Run on mount + whenever tab gains focus
  useEffect(() => {
    syncLogs();
    window.addEventListener("focus", syncLogs);
    return () => window.removeEventListener("focus", syncLogs);
  }, []);

  // 📊 Derived stats (always recalculated)
  ...
  const avgStory = total
    ? Math.round(
        logs.reduce((sum, l) => sum + (l.storyProb || 0), 0) / total
      )
    : 0;
  ...
  const getFlags = (log) => {
    ...
    if (flags.length === 0)
      flags.push("sufficient detail and consistency");
    return flags;
  };
    ...
    <section className="dashboard-wrapper">
      ...
      <div className="card-grid">
        ...
        <div className="card">
          ...
          <p>Average story confidence: <b>{avgStory}%</b></p>
          <p className="muted">
